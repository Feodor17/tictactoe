...
def start_new_game():
    ...
    #Создаем цикл для ходов игрока и программы, по условию пока кто-то не выиграл
...
def init_desk():
    # Создадим пустую. заполним пустую матрицу пустыми значениями, чтоб отличать заполенные уже ячейки от пустых
    matrixList = []
    for i in range(3):
        yList = ["-","-","-"]
